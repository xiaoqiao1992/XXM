...
@interface QQWKWebviewTool()<SKStoreProductViewControllerDelegate,UINavigationControllerDelegate, UIImagePickerControllerDelegate>
/** 本地临时图片地址数组 */
@property (nonatomic, strong)       NSMutableArray * tempImagePathArray;
@end
@implementation QQWKWebviewTool
static __weak QQWKWebviewBaseController * _webVC;
static NSString * _webViewCallBackMethod;
static QQWKWebviewTool *_tool;
...
+ (void)openURLFromVc:(UIViewController *)fromVc withUrl:(NSURL *)url
{
    ...
    if ([redirectionUrlScheme isEqualToString:@"itms-appss"]) {
        [QQWKWebviewTool jumpToAppStoreFromVc:fromVc withUrl:url];
        return;
    }
...
{
    if ([fromVc isKindOfClass:QQWKWebviewBaseController.class]) {
        _webVC = (QQWKWebviewBaseController *)fromVc;
