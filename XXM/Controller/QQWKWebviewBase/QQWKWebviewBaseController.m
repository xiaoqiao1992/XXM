//
//  QQWKWebviewBaseController.m
//  XXM
//
//  Created by QQ on 2021/12/14.
//

#import "QQWKWebviewBaseController.h"
// 定位相关
#import <CoreLocation/CoreLocation.h>
#import "NSDictionary+Category.h"
#import "QQWKWebviewTool.h"
@interface QQWKWebviewBaseController ()
/** locationMgr */
...
@implementation QQWKWebviewBaseController
...
- (void)loadView
{
    ...
    _webView = [[QQWKWebView alloc] initWithFrame:CGRectZero configuration:config];
    [config.userContentController addScriptMessageHandler:_webView name:_webViewAppName];
    _webView.scrollView.scrollEnabled = _scrollEnabled;
    _webView.QQWKWebViewDeelegate = self;
...
- (void)dealloc {
    ...
    // 移除选择过的本地图片
    [QQWKWebviewTool removeTempImages];
}
...
- (UIProgressView *)progressView
{
    if (_progressView == nil) {
        ...
        CGFloat progressViewY = kNavigationHeight;
        if (self.useWebNavigationBar) {
            progressViewY = [UIApplication sharedApplication].statusBarFrame.size.height;
        }
        
        CGRect frame = CGRectMake(0, progressViewY, kSCREEN_WIDTH, 5);
...
- (UIView *)HUD{
    if (_HUD == nil) {
        _HUD = [UIView new];
        _HUD.frame = CGRectMake(50, 0, kSCREEN_WIDTH - 100, kSCREEN_HEIGHT);
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            tip.frame = CGRectMake(X, kSCREEN_HEIGHT/2 - 100, width, height);
        });
...
- (void)xy_webView:(QQWKWebView *)webView didReceiveScriptMessage:(QQScriptMessage *)message
{
    ...
    //打开新页面
        ...
        if (url.length) {
            QQWKWebviewBaseController *webViewController = [[QQWKWebviewBaseController alloc] init];
    ...
    //打开相册[目前只支持选择一张]
        ...
        if ([src containsObject:@"album"]) {
            [alert addAction:[UIAlertAction actionWithTitle:@"从相册选择" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                ...
                [QQWKWebviewTool chooseImageFromVC:self sourceType:UIImagePickerControllerSourceTypePhotoLibrary callBackMethod:message.callback];
            }]];
        }
        if ([src containsObject:@"camera"]) {
            [alert addAction:[UIAlertAction actionWithTitle:@"从相机选择" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                ...
                {
                    [QQWKWebviewTool chooseImageFromVC:self sourceType:UIImagePickerControllerSourceTypeCamera callBackMethod:message.callback];
                }
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    if (!HTTPScheme) {
        ...
        if (!bSucc) {
            // 不可识别的，提示未安装客户端
            [QQWKWebviewTool openURLFromVc:self withUrl:navigationAction.request.URL];
            // 禁止通过,当前请求
            decisionHandler(WKNavigationActionPolicyCancel);
            return;
        }
